...
if st.session_state.page == "main":
    ...
    if temp > 37.0: insensible_total *= (1 + 0.15 * (temp - 37.0))
    if r_temp > 30.0: insensible_total *= (1 + 0.175 * (r_temp - 30.0))
    ...
    # --- 6. PDF生成・保存 ---
    st.markdown("---")
    if st.button("📄 PDFレポートを生成・保存", use_container_width=True, key="btn_final_pdf"):
        report_data = {
